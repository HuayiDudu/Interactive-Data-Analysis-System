...
- 默认数据库文件：`analysis.db`（由 `config.py` 中 `DATABASE_PATH` 配置）
...
> **注意**：当前使用 `SQLiteRepository` 而非 `FileRepository`。`config.py` 中的 `DATABASE_PATH` 控制数据库文件路径，默认生成 `analysis.db`。
...
| `handleAnalyze(datasetId)` | `analyze.js` | **✅ 前端已实现，后端待完成** |
| `populateAlgorithmParams(algorithm)` | `analyze.js` | **✅ 前端已实现，后端待完成** |
