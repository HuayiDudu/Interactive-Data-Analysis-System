...
```javascript
const AppState = {
    currentDatasetId: null,   // 当前操作的数据集 ID
    currentColumns: [],       // 当前数据集的列名
    isProcessing: false,      // 是否在处理中（用于防重复点击）
};
...
function showStep(stepId) {
    document.querySelectorAll("[data-step]").forEach(el => {
        el.style.display = el.dataset.step === stepId ? "block" : "none";
    });
}
...
function onUploadSuccess(data) {
    ...
    AppState.currentColumns = data.columns;
...
document.getElementById("btn-clean").addEventListener("click", async function() {
    if (!AppState.currentDatasetId) return;

    const params = collectCleanParams();
    params.dataset_id = AppState.currentDatasetId;
...
A: 你维护的 `AppState.currentDatasetId` 会在每次操作后更新（清洗和分析会生成新的 dataset_id），确保导出时使用的是最新的 ID。
