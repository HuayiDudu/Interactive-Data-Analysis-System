...
【业务层】 services/data_service.py            ← 你在这里实现业务逻辑
    ↓ DataRepository 抽象接口
【数据访问层】 repositories/file_repo.py        ← 你来实现具体存储
```
...
| `repositories/file_repo.py` | **实现** | MVP 文件仓库实现（CSV 临时文件）|
...
def export():
    ...
    repo = current_app.data_service.repo  # 通过 service 获取 repo
    df = repo.load_data(ref)
...
你实现 `FileRepository` 时需要遵循 `DataRepository` 抽象接口（由项目负责人维护）：

```python
# repositories/base.py — 你实现 file_repo.py 时需要实现的接口
...
你的 `FileRepository` 会被其他模块的 Service 通过这个接口调用，确保方法签名完全匹配。
