...
【业务层】 services/data_service.py            ← 你在这里实现业务逻辑
    ↓ DataRepository 抽象接口
【数据访问层】 repositories/file_repo.py        ← 通用组件（我负责）
```
...
| `repositories/file_repo.py` | 只读引用 | 通用组件，由我实现，你通过上层 API 调用 |
...
## 六、依赖的通用组件（由我负责，你只需了解接口）

你的代码通过以下接口与通用组件交互：

```python
# Repository 接口（你调用 repo 的方法）
repo.save_data(df)        # → DatasetRef
repo.load_data(ref)       # → DataFrame
repo.delete_data(ref)     # → None
